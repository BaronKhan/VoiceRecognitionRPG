...
\subsubsection{Generating Sentences from Vector Representations}
...
\\
\\
To access the WordNet lexical database, the JWI (Java WordNet Interface) will be used because, according the the author Mark Finlayson, it is the "highest-performance, widest-coverage, easiest-to-use library available" \cite{RefWorks:19}. For the POS tagger, the Stanford Log-Linear Part-of-Speech Tagger will be used \cite{RefWorks:51}, as it includes a Java API.
\\
\\
Once the general I/O and systems are created, a skeleton for the game state will be written, where the NLP and gameplay systems will be written. the battle system will be developed first with a simple demo consisting a battle with a single enemy. Simple commands will be implemented (such as "attack" and "heal") without involving items. These actions will be hard-coded initially into a hash map before the NLP systems (the POS tagger, WordNet systems, etc.) are added to the system, so more phrases can be used with different verbs but similar meanings to the hard-coded strings.
\\
\\
...
The success of this project will be measured by the improvements to the user experience and the game development process that this new NLP system will provide. Users should be able to 
