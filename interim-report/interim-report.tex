...
One feature from the proposed design of the RPG is to be able to generate text-based descriptions of the current situation that the player is in based on objects around them. For instance, a description could be, "You are in a room lit by a candle on a table. There is a piece of broken glass on the floor and a door in front of you." One could hard-code this sentence for each room the player visits, but if there are a lot of rooms in the game, this could become time-consuming. We would like to generate a sentence of the current situation based on sparse pieces of information (such as the items in the room, plus their locations, in a vector representation). This is the opposite of the intent inference described above, where the meaning of a sentence/phrase is extracted; we would like to be able to go in the opposite direction.
\\
\\
This is a relatively new field in NLP as it is particularly difficult. One method of sentence generation is suggested by Iyyer et al \cite{RefWorks:55}, where recursive autoencoders (a type of neural network) to generate a decomposition model of the sentence that can be used to reconstruct the sentence again. However, they found that proper nouns were not reconstructed correctly, and the model would need to be built in the first place to reconstruct the sentence.
\\
\\
Another group of researchers proposed a method of sentence generation which involves taking a sum of word embedding vectors and modelling it as a mixed integer programming problem \cite{RefWorks:54}. However, this also requires the original sentence to be de-constructed in the first place.
\\
\\
...
\\
\\
Once the general I/O and systems are created, a skeleton for the game state was written, where the NLP and gameplay systems will be written. The battle system was developed first with a simple demo consisting a battle with a single enemy. Simple commands were implemented (such as "attack" and "heal") without involving items. These actions were hard-coded initially into a hash map before the NLP systems (the POS tagger, WordNet systems, etc.) were added, so more phrases can be used with different verbs but similar meanings to the hard-coded strings.
\\
\\
Initially, only the Wu and Palmer method for semantic similarity scoring will be used (this measure calculates the ratio of the depth of the Lowest Common Subsumer to the sum of the depths of the two words). Once this works, the other semantic similarity measures (such as the Jiang and Conrath, or the Leacock and Chodorow measures \cite{RefWorks:46}) will be added, and they will all be used to calculate an average score from all the measures, to avoid any bias from using a particular measures (though, only a few will be used to reduce the amount of processing).
\\
\\
...
\\
\\
The second challenge would be to convey to the user the current situation they are in, in order to allow them to play the game and issue commands. For example, if the player is in a room filled with items that they can interact with, the user should be given a description of what's in the room. This text output cannot be hard-coded as it would be very time-consuming. Instead, the sentence could be generated from a vector of sparse information, as described above. This vector could simply contain the items in the room and their relative positions, and a sentence is generated based on the vector information.
\\
\\
As an extension, it may be possible to create a learning mechanism for the system. For example, the user may be trying to say a phrase that is not being detected by the proposed NLP system. In this case, there should a a 'training' mode which allows the user to define new 'helper' phrases. For instance, if the player says "I want to regenerate" but this is not mapped to a healing intent (as "heal" and "regenerate" not very similar semantically), the player should be able to train the system to be able to accept this input, e.g. by saying, "regenerate means heal", and this will map the verb "regenerate" to the "heal" intent.
\\
\\
...
\\
\\
In terms of evaluating the ease of development and integration of the voice commands in games, this could be evaluated by measuring game development time to implement two versions of the same RPG, one using the proposed design, and one without. Unfortunately, it does not seem feasible to measure this within the time frame of this project.
