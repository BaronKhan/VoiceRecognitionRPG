...
public class Main {
    ...
    public static void run() throws IOException {
        ...
        if (!sUseHybrids) {
            for (SemanticSimilarity.SimilarityMethod method : methods) {
                ...
                String fileName = Environment.getExternalStorageDirectory()+ "/results" + (method.name().replace("METHOD_", "")) + ".csv";
                Log.d("Main", "Running test suite using " + method.name() + " method...");
                runTestSuite(fileName, method, null);
                Log.d("Main", "Finished test suite for " + method.name() + " method...");
            }
        } else {
            ...
            for (SemanticSimilarity.SimilarityMethod method1: methods) {
                for (SemanticSimilarity.SimilarityMethod method2: methods) {
                    ...
                    Log.d("Main", "Running test suite using "+method1.name()+" and "+method2.name()+" method...");
                    runTestSuite(fileName, method1, method2);
                    Log.d("Main", "Finished test suite for "+method1.name()+" and "+method2.name()+" method...");
                }
            }
        }
    }
    ...
    private static void loadWordNetAndPOSTagger() throws MalformedURLException {
        VoiceProcess.loadTagger(Environment.getExternalStorageDirectory()
                + "/english-left3words-distsim.tagger");
        File dictFile = new File(Environment.getExternalStorageDirectory().getPath()+"/dict/");
        if (dictFile.exists()) {
            Log.d("Main", "Found WordNet database");
        } else {
            Log.d("Main", "Could not find WordNet database. Path = "+dictFile);
            System.exit(0);
        }
                                     ...
                                     SemanticSimilarity.SimilarityMethod method2) throws IOException
    {
        ...
        if (file.exists()) {
            Log.d("Main", "Found existing "+fileName+". Skipping...");
            return;
        } else {
            Log.d("Main", "Writing to "+fileName);
        }
        ...
        catch (Exception e) { Log.d("Main", e.getMessage()); }
        ...
        Log.d("Main", "Final Results");
        Log.d("Main", "Total time = "+totalTime+" ms");
        Log.d("Main", "Average of Scores = "+new DecimalFormat("##.00").format(avgScore)+"%");

        writer.close();
        Log.d("Main", "Created CSV file: "+fileName);
    }
    ...
    private static void runTests(CSVWriter writer, GlobalState state, String testName, List<Pair<String, String>> tests) {
        ...
        Log.d("Main", "Running tests for "+testName);
        ...
        for (Pair<String, String> test : tests) {
            ...
            Log.d("Main", "Result for "+testName+" test: "+input+" : "+gameRecord[2] +
                    (passed ? "" : " ---> "+result));
        }
        ...
        writer.writeNext(new String[] {""});
        Log.d("Main", "Finished tests for "+testName);
        Log.d("Main", "Total time = "+totalTime+" ms");
        Log.d("Main", "Score = "+new DecimalFormat("##.00").format(score)+"%");
    }
