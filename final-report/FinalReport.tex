...
\\
\\
We explore and evaluate various methods for improving the voice recognition system with the aim of decreasing the developer workload. These include using semantic similarity methods, adaptive and user-controlled learning mechanisms, and various other NLP techniques.}
...
\\
\\
The interface is within an Android \texttt{Activity} class. When the activity is created, it also downloads a tar ball containing a WordNet database and unzips it on the device's local storage. See the Appendix \ref{appendix:download} for a snippet of the download code.
\\
\\
The \texttt{MainActivity} class contains a \texttt{VoiceControl} classes that handles the speech-to-text (STT) I/O. It contains a \texttt{SpeechRecognizer} class that 
