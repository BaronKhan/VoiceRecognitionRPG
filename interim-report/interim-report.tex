...
\\
\\
The proposed design for the RPG aims to achieve the following innovations related to voice-recognition game design:
\begin{itemize}
...
Below is a Gantt Chart showing the expected timeline of the project as of 26th January 2018:
\begin{center}
\includegraphics[width=\textwidth, angle=0]{gantt-chart.pdf}
\end{center}

\subsection{Preliminary Work}

The first step was to research games using voice recognition and different libraries and papers, as well as develop a better understanding of natural language processing. While researching, general development commenced to get the I/O and libraries working correctly. For the speech-to-text engine, Google's open-source API is being used as it is easy to use for Android development \cite{RefWorks:50}.
\\
\\
To access the WordNet lexical database, the JWI (Java WordNet Interface) is being used because, according the the author Mark Finlayson, it is the "highest-performance, widest-coverage, easiest-to-use library available" \cite{RefWorks:19}. For the POS tagger, the Stanford Log-Linear Part-of-Speech Tagger was chosen \cite{RefWorks:51}, as it includes a simple-to-use Java API.
\\
\\
Once the general I/O and systems were created, a skeleton for the game state was written, where the NLP and gameplay systems were written. The battle system was developed first with a simple demo consisting of a battle with a single enemy. Simple commands were implemented (such as "attack" and "heal") without involving items. These actions were hard-coded initially into a hash map before the NLP systems (the POS tagger, WordNet systems, etc.) were added. With these systems now in place, more phrases can be used with different verbs but similar meanings to the hard-coded strings. See the Appendix for screenshots of the prototype developed.

\subsection{Future Work}

Initially, only the Wu and Palmer method for semantic similarity scoring will be used (this measure calculates the ratio of the depth of the Lowest Common Subsumer to the sum of the depths of the two words). Once this works, the other semantic similarity measures (such as the Jiang and Conrath, or the Leacock and Chodorow measures \cite{RefWorks:46}) will be added, and they will all be used to calculate an average score from all the measures, to avoid any bias from using a particular measure (though, only a few will be used to reduce the amount of processing).
\\
\\
The voice recognition flexibility will be tested throughout the project. Eventually, items will be added to the game, and the exploration gameplay will be developed. The game will alternate between battles and exploration.
\\
\\
There are several challenges to this project that aim to make the NLP system more robust. The first one is to add the ability to infer which item the user is referring to based on a given description, as described previously. This would be achieved by giving items properties, such as sharpness or flammability, and applying the semantic similarity process to these adjectives as well.
\\
\\
The second challenge would be to convey to the user the current situation they are in using a natural language output, in order to allow them to play the game and issue commands. For example, if the player is in a room filled with items that they can interact with, the user should be given a description of what's in the room. This text output cannot be hard-coded as it would be very time-consuming. Instead, the sentence could be generated from a vector of sparse information, as described above. This vector could simply contain the items in the room and their relative positions, and a sentence is generated based on the vector information.
\\
\\
As an extension, it may be possible to create a learning mechanism for the system. For example, the user may be trying to say a phrase that is not being detected by the proposed NLP system. In this case, there should be a 'training' mode which allows the user to define new 'helper' phrases. For instance, if the player says "I want to regenerate" but this is not mapped to a healing intent (as "heal" and "regenerate" are not very similar semantically), the player should be able to train the system to be able to accept this input, e.g. by saying, "regenerate means heal", and this will map the verb "regenerate" to the "heal" intent.
\\
\\
Another possible extension is to be able to allow a developer to easily design levels for the game. For instance, if the developer would like to create a room with objects, they could type a string description of the objects in the room, and then using POS-tagging and inference, a room full of the given items can be generated effortlessly.

\newpage
\section{Evaluation Plan}

The success of this project will be measured by the improvements to the user experience and the game development process that this new NLP system will provide. Users should be able to execute intents in a flexible manner with little to no frustration, while developers should be able to easily implement new voice intents with little effort while still being able to cover a wide range of acceptable phrases.
\\
\\
For the player experience evaluation, a quantitative approach would be to calculate the proportion of successful inputs given by the user to the total number of inputs given as they progress through the game. We want this to be as high as possible, and to avoid the user becoming frustrated if their command in not accepted. We would like the player to explore as many different acceptable phrases as possible to ensure the system can work in many different circumstances.
\\
\\
A qualitative approach would be to perform surveys on players to measure their satisfaction from the voice recognition system. Questions would be of the form, "Was it easy to issue voice commands?", "Did you understand what actions were available to you in each scenario?", and, "Did the use of a voice recognition scheme improve your enjoyment of the game?". Each question would be answered with a score out of five or ten.
\\
\\
In terms of evaluating the ease of development and integration of the voice commands in games for the developer, this could be evaluated by measuring game development time to implement two versions of the same RPG, one using the proposed design, and one without. Unfortunately, it does not seem feasible to measure this within the time frame of this project, though simple examination of the size of the source code of the game could be performed; the smaller the amount of code required to implement a system, the better.
...
Below are a few screenshots from the current prototype of the game. It involves the player battling a troll, with the "attack" and "heal" intents available to the player (note that items are currently not supported yet). The first two screenshots demonstrate how the player can issue an attack in several ways (despite only the intent, "attack" being hard-coded in the source code):

\includegraphics[scale=0.2]{Screenshot_20180124-163540.png}
\includegraphics[scale=0.2]{Screenshot_20180124-163553.png}
\newpage
The next screenshot demonstrates how input is ignored if it does not match any existing intent while the last screenshot demonstrates the heal intent.
\break
\break
\includegraphics[scale=0.2]{Screenshot_20180124-163621.png}
\includegraphics[scale=0.2]{Screenshot_20180124-163634.png}
