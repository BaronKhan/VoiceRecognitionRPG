...
\textbf{When adding voice commands to a video game, a developer may find themselves adding each variation of a phrase that a player can say, and mapping them to intents within the game. It becomes more work to include varying phrases of the same intent as the developer would have to include every permutation of the phrase (for which there could be infinitely many).
\\
\\
This project presents a text-based role-playing game (RPG) on Android which allows the player to issue actions within the game using their voice. The game uses a new voice recognition system that aims to make it easier for the developer to add voice commands without hard-coding the phrases, and works completely offline with no cloud processing required. The system uses the WordNet database created by Princeton University to process voice commands.
\\
\\
...
\\
\\
This project also explores other areas for easing the development of the RPG using WordNet, such as automatically assigning properties to objects, and room generation from text descriptions.}
...
See Appendix \ref{appendix:process-input-pseudo} for the pseudocode of the \texttt{processInput()} method. For the full Java method, refer to the source code in the project's repository.
\\
\\
...
\\
\\
The WS4J library is packaged with its own implementation of \texttt{ILexicalDatabase} interface, called \texttt{NictWordNet} (which accesses a Japanese WordNet database similar to the Princeton WordNet \cite{RefWorks:122}). Unfortunately, due to its use of SQLite methods to access the database, the implementation is not compatible with the Android SDK, and could not be used.
\\
\\
...
The use of room generation in this project aims to decrease the workload of the developer and designer as much as possible, using semantic similarity techniques discussed above.
...
\\
\\
The semantic similarity engine used in the voice recognition system is used again here to create objects using synonyms in the room description (e.g. the sentence, ``There is a utensil in the room" may add a spoon to the room since ``spoon" is semantically similar to ``utensil", and a spoon is in the object file).
\\
\\
...
\begin{center}
  ...
  \caption{A bar chart showing the score for each domain and the final (average) score for each semantic similarity method in the evaluation project.}
...
\\
\\
Methods which produced high accuracy such as LCH and WUP also have fast performance, so they are ideal methods to use in the application. It should also be noted that multi-threading the system may be able to, at least, double the speed of processing.
...
The large increases in the number of tests that pass when adding the semantic similarity engine or the suggestions mechanism indicates that they have a more sizeable impact compared to the other features (i.e. most variations of commands can only be detected if these features are enabled).
